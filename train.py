...
train_datasets = datasets.ImageFolder(os.path.join(data_dir,'train'), transforms["train"])
valid_datasets = datasets.ImageFolder(os.path.join(data_dir,'valid'), transforms["valid"])
...
for epoch in range(num_epochs):
    ...
    #    imshow(inputs.cpu().data[0], title='예측 결과: ' + class_names[preds[0]])
        ...
        print('valid - #{} Loss: {:.4f} Acc: {:.4f}% Time: {:.4f}s\n'.format(epoch,epoch_loss, epoch_acc, time.time() - start_time)) 


# save model
os.makedirs('./weight',exist_ok=True)

param_PATH = './weight/best_epoch_param.pt'
PATH = './weight/best_epoch_model.pt'
...
torch.onnx.export(model, dummy_input, "./weight/animalface_detector.onnx",export_params=True)
...
image = transforms['test'](image).unsqueeze(0).to(device)
